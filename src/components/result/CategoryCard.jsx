      ...
      <ul className="mt-3 space-y-2.5 border-t border-zinc-100 pt-3">
        {items.map((item) => (
          <li key={item.id} className="text-sm leading-relaxed text-zinc-800">
            ...
            <span className="block pl-2 text-xs text-zinc-500">└ {item.legalBasis}</span>
          </li>
        ))}
      </ul>
      <button
        ...
        className="mt-3 text-left text-sm font-semibold text-toss underline decoration-toss/40"
      >
        {open ? '개선 가이드 접기' : '[이 영역 개선 가이드 보기]'}
      </button>
      {open && (
        <p className="mt-2 rounded-lg bg-zinc-50 px-3 py-2 text-sm leading-relaxed text-zinc-800">
          {items[0]?.improvementGuide}
        </p>
      )}
      <Link
        to={`/contact?concerns=${encodeURIComponent(label)}`}
        className="mt-3 inline-block text-sm font-semibold text-zinc-600 underline decoration-zinc-400"
      >
        이 영역 상담 신청하기 →
      </Link>
    </div>
